...
abstract class IAgentRepo {
  ...
  /// 响应式订阅指定 agent 的数据变化。
  ///
  /// **契约**(Drift + InMemory 实现均遵守):
  /// - 订阅时立即 emit 当前行(seed event),如果 localId 不存在则 emit null
  /// - 后续每次该 agent 写入 emit 一次;tombstoned agent (removed_at != null)
  ///   正常 emit,由调用方通过 `agent.isRemoved` 判断
  /// - 不存在的 localId 立即 emit null 并保持 open(等待后续创建)
  ///
  /// **Drift 实现的 reactivity gap**:`DriftAgentRepo.syncFromGateway` 的
  /// tombstone / revive 步骤走 `customStatement`(SQLite 批量 UPDATE 避免 N+1),
  /// **不**触发 Drift 的 `.watchSingleOrNull()` 订阅 —— 订阅者不会收到
  /// tombstone 翻转通知。当前通过 `agentSyncTickerProvider` 驱动的
  /// `ChatViewModel.refreshAgent` 双保险弥补;未来若给 agents 表加新的
  /// `.watch()` 消费者,需评估是否要改 Drift typed update 以保证订阅一致性。
  ///
