...
void main() {
  ...
  group('Update', () {
    ...
    test('编辑名称保持相同时跳过唯一性检查', () async {
      ...
      verify(() => instanceRepo.save(any())).called(1);
      verifyNever(() => instanceRepo.nameExists(any(), excludeId: any(named: 'excludeId')));
    });

    test('编辑修改名称时检查唯一性（排除自身）', () async {
      when(() => instanceRepo.getById('inst-1'))
          .thenAnswer((_) async => existingInstance);
      when(() => instanceRepo.nameExists('新名称', excludeId: 'inst-1'))
          .thenAnswer((_) async => false);
      when(() => gatewayClient.testConnection(any()))
          .thenAnswer((_) async => true);
      when(() => instanceRepo.save(any()))
      ...
      verify(() => instanceRepo.nameExists('新名称', excludeId: 'inst-1')).called(1);
    });

    test('编辑时新名称被其他实例占用应失败', () async {
      when(() => instanceRepo.getById('inst-1'))
          .thenAnswer((_) async => existingInstance);
      when(() => instanceRepo.nameExists('已被占用', excludeId: 'inst-1'))
          .thenAnswer((_) async => true);
    ...
    test('编辑不存在的实例应失败', () async {
      when(() => instanceRepo.getById('nonexistent'))
