...
void main() {
  group('DI Providers', () {
    ...
    test('repository providers throw without database override', () {
      ...
      expect(
        () => container.read(instanceRepoProvider),
        throwsUnimplementedError,
      );
      expect(() => container.read(agentRepoProvider), throwsUnimplementedError);
      expect(
        () => container.read(messageRepoProvider),
        throwsUnimplementedError,
      );
      expect(
        () => container.read(conversationRepoProvider),
        throwsUnimplementedError,
      );
    });

    test(
      'use case providers resolve through the full DI graph (in-memory DB)',
      () {
    ...
    test(
      'databaseProvider override disposes the DB when container is torn down',
      () async {
        final memDb = db.AppDatabase(NativeDatabase.memory());
        ...
        expect(() => memDb.getAllInstances().get(), throwsA(anything));
      },
    );
  });
}
