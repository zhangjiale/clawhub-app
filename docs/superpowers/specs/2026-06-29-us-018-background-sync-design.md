...
后台 isolate:
  handlePulledMessages(messages)
    → 对每条 msg: evaluateNotification
      → [ShowDecision]: pendingRepo.insert(msg)   // persistent 去重；DND 到期由 flushDndSummary show
      → [DndSuppressedDecision]: pendingRepo.insert(msg)  // 静默入队，等 NotificationCoordinator DND Timer 触发 flushDndSummary
...
| **`WidgetsBindingObserver.onPaused` SharedPreferences 写无 await 屏障** | 主 isolate 走 `onPaused` → prefs 写 `false` 是异步的，若 workmanager 在写完前已启动读到 `true` → skip 本次（保守方向，浪费 15min 窗口） | 文档化异步语义；用 `SharedPreferences.setBool`（内部 flush 由插件决定，Android 同步 / iOS 异步）。接受现状作为 best-effort。下次 workmanager 调度时 prefs 已 flush |
